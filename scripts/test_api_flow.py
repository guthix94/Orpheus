...
def main() -> None:
    ...
    stopped = req("POST", f"/api/lessons/{lesson_id}/stop", {
        "audio_file_path": f"audio/{lesson_id}.webm",
    })
    ...
    check("audio_file_path stored", stopped["audio_file_path"] == f"audio/{lesson_id}.webm")

    # ---- Step 8: Verify lesson persisted ----
    print(f"\n{INFO} Step 8: Verify lesson persisted (GET)")
    persisted = req("GET", f"/api/lessons/{lesson_id}")
    check("GET /api/lessons/{id} returns correct lesson", persisted["id"] == lesson_id)
    check("status still 'processing'", persisted["status"] == "processing")
    check("student_id correct", persisted["student_id"] == student_id)

    # ---- Step 9: List lessons for student ----
    print(f"\n{INFO} Step 9: List lessons filtered by student")
    ...
    # ---- Step 10: Send parent message ----
    print(f"\n{INFO} Step 10: Send parent message")
    ...
    # ---- Step 11: Verify communication log ----
    print(f"\n{INFO} Step 11: Verify communication log")
    ...
    # ---- Step 12: Double-stop should fail (409) ----
    print(f"\n{INFO} Step 12: Double-stop returns 409")
    try:
